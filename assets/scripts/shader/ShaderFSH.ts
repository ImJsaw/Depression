...
const ShaderFSH = [
  ...
  },
  {
    ...
    void main () {
      float offset = 0.04 * noise(iTime);

      vec4 maskR = texture2D(texture, uv0 + vec2(1, 1) * (offset - 0.02));
      vec4 maskB = texture2D(texture, uv0 - vec2(1, 1) * (offset - 0.02));
      vec4 mask = texture2D(texture, uv0);
